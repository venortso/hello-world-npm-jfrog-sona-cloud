...
# 
# 2. ARTIFACTORY_URL, ARTIFACTORY_USER, ACCESS_TOKEN, ARTIFACTORY_REPO
#
...
export ARTIFACTORY_URL="https://envorso.jfrog.io/artifactory"
export ARTIFACTORY_USER="vito.luong@envorso.com"
export ACCESS_TOKEN="eyJ2ZXIiOiIyIiwidHlwIjoiSldUIiwiYWxnIjoiUlMyNTYiLCJraWQiOiJodHVPR0d4ZnhZR0x6SWtmNWdicExGdU1QRk1JNVl2RmRPVVdaWWJocDIwIn0..ATdpxD_N-7CZ1t7-SaAAduhKtsB9emtiD1OLRhqvgVE38zs5zED0mGNXII-Yv5hSNMSblB82sLGDWs2l2h7d4Rdc8wVNTVOFoezhs7haLSgGfsi5prAvR_V2eT-qrf1GKO00rbKGOunIOlvdczBAgwt8BrBQF_y2oj8DD7xoTgkYisPVGGp2AcmSR21MOvBfsQLpOmdBvccZQE_pbnUZab9p6qQwL5QKTtq8yUGXNC3RyNMiIf3dM7PF_pAYbgUgXKUuPiXR_abfj0JiTYr8O_w2z-RktLj2TyEDke4MH_CF8Eov_HwYiIJHQhn0tKu8ORFeJA6JKVvJEWdU4Y7QZg"
export REPO_DEPLOY="venortso-npm-challenge-local"
export REPO_RESOLVE="venortso-npm-challenge-remote"
...
if [ -z $CI_BUILD_NUM ]; then
	...
	jf npm-config --global=true --repo-resolve=venortso-npm-challenge-remote --repo-deploy=venortso-npm-challenge-local
  ...
  jf npm publish --build-name=$CI_BUILD_NAME --build-number=$CI_BUILD_NUM --flat=false --server-id=jf1 
	...
	# read env vars
	jf rt bce venortso$CI_BUILD_NAME $CI_BUILD_NUM

	# Publish build info
	jf rt bp venortso$CI_BUILD_NAME $CI_BUILD_NUM --server-id=jf1
